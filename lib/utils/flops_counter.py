...
import torch
import numpy as np
from torchvision.ops import DeformConv2d as ConvOffset2d
...
def is_supported_instance(module):
    if isinstance(module, torch.nn.Conv2d) or isinstance(module, torch.nn.ReLU) \
       ...
       or isinstance(module, torch.nn.Upsample) or isinstance(module, ConvOffset2d):
        return True
...
def convoffset2d_flops_counter_hook(conv_module, input, output):
    # Can have multiple inputs, getting the first one
    input = input[0]
...
def add_flops_counter_hook_function(module):
    if is_supported_instance(module):
        ...
        elif isinstance(module, ConvOffset2d):
            handle = module.register_forward_hook(convoffset2d_flops_counter_hook)
        else:
